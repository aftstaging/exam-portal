...
export default function Catalogue({ kind = "mock-exams" }: { kind?: "mock-exams" }) {
  ...
  const mocks = trpc.catalogue.mockExams.useQuery(undefined, { retry: false, enabled: !isStaff });
  const productsQuery = trpc.catalogue.products.useQuery(undefined, { retry: false, enabled: !isStaff });
  const staffCatalogue = trpc.admin.staffCatalogue.useQuery(undefined, { retry: false, enabled: isStaff });
  const publish = trpc.admin.updateProductStatus.useMutation({
    onSuccess: () => { toast.success("Product status updated"); utils.admin.staffCatalogue.invalidate(); },
    onError: (error) => toast.error(error.message),
  });
  const [filter, setFilter] = useState("all");
  const staffData = staffCatalogue.data;
        ...
        {loading ? <div className="mt-8 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{[1, 2, 3].map((item) => <div key={item} className="h-80 animate-pulse rounded-2xl border border-white/10 bg-[#120730]" />)}</div> : visibleProducts.length === 0 ? <Card className="mt-8 border-[#00e5ff]/30 bg-[#120730]"><CardContent className="p-10 text-center"><ShoppingBasket className="mx-auto h-10 w-10 text-[#00e5ff]" /><h2 className="mt-5 text-xl font-bold text-white">No products match this filter</h2><p className="mt-2 text-sm text-[#c4b5fd]">Choose another store filter to view the available AFT practice products.</p></CardContent></Card> : <div className="mt-8 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{visibleProducts.map(({ product, qualification }) => {
          ...
          return <Card key={product.id} className="group overflow-hidden border-white/10 bg-[#120730] transition duration-200 hover:-translate-y-1 hover:border-[#00e5ff]/50">
            <CardContent className="p-0"><div className="relative h-44 overflow-hidden"><img src={featuredImage} alt="" className="h-full w-full object-cover transition duration-300 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-[#120730] via-transparent to-transparent" /></div><div className="p-6">
              ...
              {isStaff && (
                <div className="mt-3 flex items-center gap-2 rounded-xl border border-white/10 bg-[#0c0524]/60 px-3 py-2">
                  <Badge className={`${product.status === "published" ? "bg-[#102b36] text-[#00ff88]" : product.status === "archived" ? "bg-[#18093c]/60 text-white/55" : "bg-[#18093c]/60 text-[#f4c44e]"}`}>{product.status ?? "draft"}</Badge>
                  {product.status !== "published" ? (
                    <Button size="sm" variant="outline" className="h-7 border-[#00ff88] px-2 text-[11px] text-[#00ff88]" disabled={publish.isPending} onClick={() => publish.mutate({ productId: product.id, status: "published" })}>Publish</Button>
                  ) : (
                    <Button size="sm" variant="outline" className="h-7 border-[#ff8278]/50 px-2 text-[11px] text-[#ff8278]" disabled={publish.isPending} onClick={() => publish.mutate({ productId: product.id, status: "archived" })}>Archive</Button>
                  )}
                </div>
              )}
