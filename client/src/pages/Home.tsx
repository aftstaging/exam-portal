...
function ExamShell({ screen, setScreen }: { screen: string; setScreen: (next: string) => void }) {
  ...
  const resolveTaskResource = (kind: "email" | "reference" | "pre_seen" | "formulae") => {
    const rows = examResourcesQuery.data?.filter((item) => item.kind === kind && (item.sectionNumber === currentSection || item.sectionNumber == null)) ?? [];
    const present = (item: typeof rows[number]) => (kind === "email" ? Boolean(item.email || (item.hasFile && item.mimeType !== "application/octet-stream")) : item.hasFile && item.mimeType !== "application/octet-stream");
