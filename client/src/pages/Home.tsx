        ...
        {isIntro && (
          <Card className="exam-card mx-auto max-w-5xl">
            <CardContent className="p-9">
              ...
              <Separator className="my-7" /><StructuredText className="max-w-3xl text-lg leading-8 text-[#c4b5fd]" text={sectionIntroduction} />
              {sectionScenario && <div className="mt-5 rounded-xl border border-[#00e5ff]/30 bg-[#18093c] p-5"><div className="text-xs font-bold uppercase tracking-[.16em] text-[#00e5ff]">Extra notes</div><StructuredText className="mt-2 text-sm leading-6 text-[#c4b5fd]" text={sectionScenario} /></div>}
...
function ProtectedResourceList({ productId }: { productId: number }) {
  ...
  return <div className="mt-5 border-t border-white/10 pt-4"><p className="text-xs font-bold uppercase tracking-[.14em] text-[#00ff88]">Protected downloads</p><Button size="sm" variant="outline" className="mt-2 w-full border-[#00e5ff] text-[#00e5ff]" disabled={zipLoading} onClick={() => void downloadZip()}>{zipLoading ? "Preparing…" : "Download"}</Button></div>;
}
...
function AppRouter() {
  ...
  const go = (next: string) => { setScreen(next); if (["mode", "debrief", "instructions", "intro", "question", "submission", "solutions", "review"].includes(next)) navigate(`/case-study/${next}${typeof window !== "undefined" ? window.location.search : ""}`); };
  ...
  if (matchedCaseStudy) {
    ...
    if (currentScreen === "review") return <Review setScreen={go} />;
...
function Review({ setScreen }: { setScreen: (next: string) => void }) {
  const routeParams = typeof window === "undefined" ? new URLSearchParams() : new URLSearchParams(window.location.search);
  const attemptId = Number(routeParams.get("attempt") || 0);
  const reviewQuery = trpc.exams.attemptAnswers.useQuery({ attemptId }, { retry: false, enabled: Boolean(attemptId) });
  const data = reviewQuery.data;
  const examTitle = data?.examTitle ?? "Case-study exam";
  const sections = data?.sections ?? [];
  return <div className="min-h-screen bg-[#0c0524]"><PublicHeader onLogin={() => startLogin("login")} /><ExamUtilityRail /><main className="container py-12"><div className="mx-auto max-w-5xl"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">Completed case study</p><h1 className="mt-2 text-4xl font-bold text-white">Review your submitted answers</h1><p className="mt-3 max-w-2xl text-[#c4b5fd]">Your {examTitle} responses are shown below exactly as they were submitted. Once you move off a task the exam cannot be revisited, so this record is read only.</p></div><div className="flex flex-wrap gap-3"><Button variant="outline" className="border-[#00e5ff] text-[#00e5ff]" onClick={() => setScreen("solutions")}>View illustrative solutions <ArrowRight className="ml-2 h-4 w-4" /></Button><Button className="aft-button" onClick={() => setScreen("debrief")}>Return to mock exam <ArrowRight className="ml-2 h-4 w-4" /></Button></div></div>{reviewQuery.isLoading ? <Card className="mt-8 border-white/10 bg-[#120730]"><CardContent className="p-10 text-center"><div className="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-[#00e5ff] border-t-transparent" /><p className="mt-4 text-sm text-[#c4b5fd]">Loading your submitted answers…</p></CardContent></Card> : !attemptId || reviewQuery.isError || !data ? <Card className="mt-8 border-[#00e5ff]/30 bg-[#120730]"><CardContent className="p-10 text-center"><LockKeyhole className="mx-auto h-10 w-10 text-[#00e5ff]" /><h2 className="mt-5 text-xl font-bold text-white">Submission review is unavailable</h2><p className="mt-2 text-sm leading-6 text-[#c4b5fd]">Open the review from the completion link of a submitted attempt.</p><Button className="aft-button mt-6" onClick={() => setScreen("debrief")}>Return to mock exam</Button></CardContent></Card> : <div className="mt-8 space-y-5">{sections.map((section) => <Card key={section.sectionNumber} className="border-white/10 bg-[#120730]"><CardContent className="p-6 sm:p-7"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="eyebrow">Task {section.sectionNumber} of {sections.length}</p><h2 className="mt-2 text-2xl font-bold text-white">{section.title}</h2></div><Badge className={section.answer.trim() ? "bg-[#102b36] text-[#00ff88]" : "bg-[#18093c] text-white/50"}>{section.answer.trim() ? `Submitted · ${section.wordCount} words` : "No answer recorded"}</Badge></div>{(section.question || section.scenario) && <StructuredText className="mt-5 max-w-4xl text-base leading-8 text-[#c4b5fd]" text={section.question ?? section.scenario} />}{section.scenario && section.question && <div className="mt-4 rounded-xl border border-[#00e5ff]/30 bg-[#18093c] p-5"><div className="text-xs font-bold uppercase tracking-[.16em] text-[#00e5ff]">Extra notes</div><StructuredText className="mt-2 text-sm leading-6 text-[#c4b5fd]" text={section.scenario} /></div>}<div className="mt-5 border-t border-white/10 pt-5"><div className="mb-2 flex items-center justify-between gap-4"><div><p className="eyebrow">Your answer</p><p className="mt-1 text-xs text-white/45">Submitted response · read only</p></div>{section.savedAt && <span className="text-xs text-white/45">Saved {new Date(section.savedAt).toLocaleString()}</span>}</div><div className="min-h-[110px] whitespace-pre-wrap rounded-xl border border-white/10 bg-[#0c0524] p-4 text-base leading-7 text-white">{section.answer.trim() ? section.answer : <span className="text-white/35">No answer was recorded for this task.</span>}</div></div></CardContent></Card>)}</div>}</div></main></div>;
}
