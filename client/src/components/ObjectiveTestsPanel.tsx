...
export default function ObjectiveTestsPanel() {
  ...
  const topics = useMemo(() => Array.from(new Set(allQuestions.map((item) => item.topic))).sort(), [allQuestions]);
  const questions = selectObjectiveQuestions(allQuestions, selectedTopics, questionCount);
  ...
  const begin = (next: Stage) => { resetAttempt(); if (next === "quiz") setQuizQuestions(shuffleQuestions(selectObjectiveQuestions(allQuestions, selectedTopics, questionCount))); setStage(next); };
  ...
  return <Shell>
    ...
    {stage === "customize" && <CustomizeScreen topics={topics} selectedTopics={selectedTopics} setSelectedTopics={(value) => { setSelectedTopics(value); setQuestion(0); }} questionCount={questionCount} setQuestionCount={(value) => { setQuestionCount(value); setQuestion(0); }} timed={timed} setTimed={setTimed} time={time} onStart={() => begin("quiz")} onBack={() => setStage("extra-instructions")} />}
...
function CustomizeScreen({ topics, selectedTopics, setSelectedTopics, questionCount, setQuestionCount, timed, setTimed, time, onStart, onBack }: { topics: string[]; selectedTopics: string[]; setSelectedTopics: (value: string[]) => void; questionCount: number | "all"; setQuestionCount: (value: number | "all") => void; timed: boolean; setTimed: (value: boolean) => void; time: string; onStart: () => void; onBack: () => void }) { return <div className="mx-auto max-w-5xl"><button className="mb-6 text-sm font-bold text-[#00e5ff]" onClick={onBack}>← Back to instructions</button><div className="grid gap-5"><Card className="border-white/10 bg-[#120730]"><CardHeader><CardTitle className="text-white">Customize your test</CardTitle><p className="text-sm text-[#c4b5fd]">Choose your question bank and exam conditions.</p></CardHeader><CardContent className="space-y-5"><label className="block text-sm font-bold text-white">Topics<span className="ml-2 text-xs font-normal text-white/50">— tick the topics you want to cover</span></label><div className="mt-2 flex flex-wrap gap-2">{topics.map((item) => { const checked = selectedTopics.includes(item); return <label key={item} className={`flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium ${checked ? "border-[#00ff88] bg-[#102b36] text-[#00ff88]" : "border-white/10 bg-[#18093c] text-[#c4b5fd] hover:border-[#00e5ff]/50"}`}><input type="checkbox" className="h-4 w-4 rounded border-white/20 accent-[#00ff88]" checked={checked} onChange={() => setSelectedTopics(checked ? selectedTopics.filter((topic) => topic !== item) : [...selectedTopics, item])} />{item}</label>; })}</div>{selectedTopics.length === 0 && <p className="text-xs text-[#c4b5fd]">All topics selected — use every topic in the bank.</p>}<label className="block text-sm font-bold text-white">Number of questions<Input type="number" aria-label="Number of questions" min={1} max={100} value={questionCount === "all" ? 60 : questionCount} onChange={(event) => { const value = Math.max(1, Math.min(100, Number(event.target.value))); setQuestionCount(Number.isNaN(value) ? 1 : value); }} className="mt-2 h-11 w-full rounded-lg border border-white/10 bg-[#18093c] px-3 text-sm font-normal text-white" /></label><div className="flex items-center justify-between rounded-xl bg-[#102b36] p-4"><div><div className="font-bold text-white">Timed mode</div><div className="text-xs text-[#c4b5fd]">{timed ? `${time} starting time` : "No countdown"}</div></div><button aria-label="Toggle timed mode" onClick={() => setTimed(!timed)} className={`h-6 w-11 rounded-full p-1 ${timed ? "bg-[#00ff88]" : "bg-[#24105c]"}`}><span className={`block h-4 w-4 rounded-full bg-white transition ${timed ? "translate-x-5" : ""}`} /></button></div><Button className="w-full aft-button" onClick={onStart}>Start test <ChevronRight className="ml-2 h-4 w-4" /></Button></CardContent></Card></div></div>; }
