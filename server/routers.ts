...
export const appRouter = router({
  ...
  resources: router({
    ...
    downloadZip: protectedProcedure.input(z.object({ productId: z.number().int().positive() })).query(({ ctx, input }) => getProtectedResourceZip(ctx.user.id, input.productId)),
  }),
  exams: router({
    ...
    attemptAnswers: protectedProcedure.input(z.object({ attemptId: z.number().int().positive() })).query(({ ctx, input }) => listAttemptAnswers(ctx.user.id, input.attemptId)),
  ...
  admin: router({
    ...
    updateContentStatus: adminProcedure.input(z.object({ kind: z.enum(["mock_exams", "resources", "objective_questions"]), id: z.number().int().positive(), status: z.enum(["draft", "published", "archived"]) })).mutation(({ ctx, input }) => updateAdminContentStatus({ ...input, userId: ctx.user.id })),
    ...
    updateProductStatus: adminProcedure.input(z.object({ productId: z.number().int().positive(), status: z.enum(["draft", "published", "archived"]) })).mutation(({ ctx, input }) => updateProductStatus({ ...input, userId: ctx.user.id })),
    ...
    updateAccessDays: adminProcedure.input(z.object({ productId: z.number().int().positive(), accessDays: z.number().int().min(1).max(3650) })).mutation(({ ctx, input }) => updateProductAccessDays({ ...input, userId: ctx.user.id })),
    updatePrice: adminProcedure.input(z.object({ productId: z.number().int().positive(), priceCents: z.number().int().min(0) })).mutation(({ ctx, input }) => updateProductPrice({ ...input, userId: ctx.user.id })),
    ...
    createExamBundle: staffProcedure.input(z.object({
      ...
      objectiveQuestions: z.array(z.object({
        ...
        correct: z.union([z.number().int().min(0), z.array(z.number().int().min(0)).max(8)]),
...
featuredImageUrl: z.string().max(1000).optional().nullable(),
    ...
    deleteExamBundle: adminProcedure.input(z.object({ mockExamId: z.number().int().positive() })).mutation(({ ctx, input }) => deleteExamBundle({ mockExamId: input.mockExamId, userId: ctx.user.id })),
  ...
  marking: router({
    ...
    stats: staffProcedure.query(({ ctx }) => listMarkerStats({ userId: ctx.user.id, role: ctx.user.role })),
    assign: adminProcedure.input(z.object({ markingId: z.number().int().positive(), markerId: z.number().int().positive() })).mutation(({ ctx, input }) => assignMarking({ ...input, role: ctx.user.role, userId: ctx.user.id })),
    release: staffProcedure.input(z.object({ markingId: z.number().int().positive(), feedback: z.string().min(1).max(100000), rubricSnapshot: z.string().max(100000).optional(), awardedPoints: z.number().int().min(0), totalPoints: z.number().int().positive() })).mutation(({ ctx, input }) => releaseFeedback({ ...input, userId: ctx.user.id, role: ctx.user.role })),
  }),
});
