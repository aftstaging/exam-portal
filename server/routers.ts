...
export const appRouter = router({
  ...
  admin: router({
    ...
    createObjectiveQuestion: staffProcedure.input(z.object({ mockExamId: z.number().int().positive(), topic: z.string().min(1).max(180), prompt: z.string().min(1).max(10000), options: z.array(z.string().min(1).max(1000)).min(2).max(8), correct: z.number().int().min(0), questionType: z.enum(["single_choice", "multiple_choice", "dropdown", "numerical", "text_input"]).optional(), explanation: z.string().max(5000).optional(), difficulty: z.enum(["easy", "medium", "hard"]), attachmentBase64: z.string().max(15_000_000).optional(), attachmentFileName: z.string().max(240).optional(), attachmentMimeType: z.string().max(120).optional() })).mutation(({ ctx, input }) => createAdminObjectiveQuestion({ ...input, userId: ctx.user.id })),
